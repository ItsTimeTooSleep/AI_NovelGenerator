...
模块功能概述
================================================================================
本模块提供Ollama本地模型API的适配器实现。

================================================================================
核心类
================================================================================
- OllamaAdapter: Ollama API适配器
...
class OllamaAdapter(BaseLLMAdapter):
    """
    适配Ollama API（使用 langchain.ChatOpenAI）

    Ollama提供OpenAI兼容的/v1/chat接口，可直接使用ChatOpenAI。
    """
    ...
    def __init__(
        ...
        self._logger.debug(
            "llm_adapters",
            f"OllamaAdapter初始化: 模型={model_name}, base_url={self.base_url}",
        )
    ...
    def invoke(self, prompt: str) -> str:
        ...
        self._logger.debug(
            "llm_adapters",
            f"OllamaAdapter调用: 模型={self.model_name}, 提示词长度={len(prompt)}",
        )
        response = self._client.invoke(prompt)
        if not response:
            self._logger.warn("llm_adapters", "OllamaAdapter未获取到响应")
            return ""
    ...
    def invoke_stream(
        ...
        self._logger.debug(
            "llm_adapters",
            f"OllamaAdapter流式调用: 模型={self.model_name}, 提示词长度={len(prompt)}",
        )
        ...
        try:
            ...
            self._logger.error("llm_adapters", f"OllamaAdapter流式调用失败: {e}")
