...
模块功能概述
================================================================================
本模块提供ML Studio本地开发环境模型API的适配器实现。

================================================================================
核心类
================================================================================
- MLStudioAdapter: ML Studio API适配器
...
class MLStudioAdapter(BaseLLMAdapter):
    """
    适配ML Studio API（使用 langchain.ChatOpenAI）

    ML Studio提供OpenAI兼容的API接口。
    """
    ...
    def __init__(
        ...
        self._logger.debug(
            "llm_adapters",
            f"MLStudioAdapter初始化: 模型={model_name}, base_url={self.base_url}",
        )
    ...
    def invoke(self, prompt: str) -> str:
        ...
        self._logger.debug(
            "llm_adapters",
            f"MLStudioAdapter调用: 模型={self.model_name}, 提示词长度={len(prompt)}",
        )
        try:
            response = self._client.invoke(prompt)
            if not response:
                self._logger.warn("llm_adapters", "MLStudioAdapter未获取到响应")
                return ""
            result = response.content

            usage_info = UsageExtractor.extract(
                response, UsageExtractor.PROVIDER_LANGCHAIN
            )
            ...
            self._logger.error("llm_adapters", f"ML Studio API调用超时或失败: {e}")
            return ""
    ...
    def invoke_stream(
        ...
        self._logger.debug(
            "llm_adapters",
            f"MLStudioAdapter流式调用: 模型={self.model_name}, 提示词长度={len(prompt)}",
        )
        ...
        try:
            ...
            self._logger.error("llm_adapters", f"MLStudioAdapter流式调用失败: {e}")
